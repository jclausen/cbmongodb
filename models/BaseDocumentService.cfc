	...
	accessors ="true"
{
	...
	any function init(){
		...
		if ( isNull( getWirebox() ) and structKeyExists( application, "wirebox" ) ) {
			application.wirebox.autowire(
				target   = this,
				targetID = getMetadata( this ).name
			);
				...
				try {
					...
					if ( structKeyExists( prop, "parent" ) ) {
						...
						if ( prop_name[ 1 ] EQ prop.parent ) {
							throw(
								"IllegalAttributeException: The parent attribute &quot;" & prop.parent & "&quot; has been been duplicated in <strong>" & getMetadata(
									this
					...
					throw(
						"An error ocurred while attempting to instantiate #prop.name#.  The cause of the exception was #error.message#"
					);
				}
			}
		}
	...
	any function set( required key, required value ){
		...
		var sget = "doc";
		var nest = listToArray(
			getDocumentPath( arguments.key ),
			"."
		);
		...
		for ( var found in normalizationFields ) {
			...
			if (
				...
				if ( normTarget.loaded() ) {
					...
					throw(
						"Normalization data for the property #mapping.name# could not be loaded as a record matching the #mapping.normalize# property value of #locate( mapping.on )# could not be found in the database."
					)
				}
			}
		}
	...
	any function normalizeOn( required string key ){
		...
		if ( !isNull( normalizationMapping ) ) {
			...
			if ( isNull( farData ) )
				throw(
					"Normalized data could not be found for model #getMetadata( this ).name# on key #arguments.key#"
				);
