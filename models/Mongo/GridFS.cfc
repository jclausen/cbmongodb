...
component name="GridFS" accessors="true" {
	...
	function init( string db = "", string bucket = "fs" ){
		...
		if ( len( arguments.db ) > 1 ) {
			...
			setGridInstance(
				jLoader
					.create( "com.mongodb.gridfs.GridFS" )
					.init(
						variables.dbInstance,
			...
			if ( structKeyExists( GridFSConfig, "imagestorage" ) ) {
				...
				if ( maxheight != img.height || maxwidth != img.width ) {
					...
					if (
						...
						throw(
							"GridFS maximum image sizes are specified but no temporary directory has been provided for processing.  Please ensure a tmpDirectory key exists in your GridFS imagestorage configuration."
						);
					}
				...
				if (
					structKeyExists(
						GridFSConfig.imagestorage,
						"metadata"
					) && GridFSConfig.imagestorage.metadata
				) {
	...
	private function isReadableImage( filePath ){
		...
		return arrayFind(
			readableImageFormats,
			lCase( listLast( filePath, "." ) )
		);
	}
