...
component name="MongoCollection" accessors="true" {
	...
	public function aggregate(
		...
		if ( !isNull( arguments.criteria ) ) {
			arrayAppend(
				proj,
				{ "$match" : arguments.criteria }
			);
		}
		if ( !isNull( arguments.projection ) ) {
			arrayAppend(
				proj,
				{ "$project" : arguments.projection }
			);
		}
	...
	public function updateMany(
		...
		return getDBCollection().updateMany(
	...
	public function createIndex(
		...
		try {
			...
			throw(
				"Index on #options[ "name" ]# could not be created.  The error returned was: <strong>#e.message#</strong>  #idxOptions.toString()#--doc:[#toMongo( operation ).toString()#]"
			);
		}
	}
	...
	public function createGeoIndex(
		...
		try {
			...
			throw(
				"Geo Index on #options[ "name" ]# could not be created.  The error returned was: <strong>#e.message#</strong> #idxOptions.toString()#--doc:[#toMongo( doc ).toString()#]"
			);
		}
	}
