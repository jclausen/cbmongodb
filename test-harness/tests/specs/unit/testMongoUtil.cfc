	...
	function run( testResults, testBox ){
		describe( "Tests Core Object Types", function(){
			...
			it( "tests newObjectIDFromID()", function(){
				...
				expect( getMetadata( MongoUtil.newObjectIDFromID( testId ) ).getCanonicalName() ).toBe(
			...
			it( "tests newIDCriteriaObject", function(){
				var testId = "53e787bd3773887239e56b17";
				expect( getMetadata( MongoUtil.newIDCriteriaObject( testId ) ).getCanonicalName() ).toBe(
					"java.lang.Class"
				);
				expect( getMetadata( MongoUtil.newIDCriteriaObject( testId )[ "_id" ] ).getCanonicalName() ).toBe(
					"org.bson.types.ObjectId"
				);
			} );
			...
			it( "creates records for our tests", function(){
				...
				describe( "Tests dbResult manipulation methods", function(){
					it( "tests the encapsulateDBResult() method", function(){
						...
						expect( getMetadata( encapsulation.asCursor() ).getCanonicalName() ).toBe(
